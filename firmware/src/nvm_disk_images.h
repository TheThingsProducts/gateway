...
#define FS_INTERNAL_FLASH_NUM_ROOT_DIRECTORY_SECTORS \
    ((FS_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT + 15) / 16) //+15 because the compiler truncates
#define FS_INTERNAL_FLASH_OVERHEAD_SECTORS                                        \
    (FS_INTERNAL_FLASH_NUM_RESERVED_SECTORS + FS_INTERNAL_FLASH_NUM_VBR_SECTORS + \
     FS_INTERNAL_FLASH_NUM_ROOT_DIRECTORY_SECTORS + FS_INTERNAL_FLASH_NUM_FAT_SECTORS)
#define FS_INTERNAL_FLASH_TOTAL_DISK_SIZE (FS_INTERNAL_FLASH_OVERHEAD_SECTORS + FS_INTERNAL_FLASH_DRIVE_CAPACITY)
#define FS_INTERNAL_FLASH_PARTITION_SIZE \
    (uint32_t)(FS_INTERNAL_FLASH_TOTAL_DISK_SIZE - 1) //-1 is to exclude the sector used for the MBR
...
#if defined(__C30__)
#error \
    "PSV only allows 32KB of memory.  The drive options selected result in more than 32KB of data.  Please reduce the number of root directory entries possible"
#endif
#endif

#if(MEDIA_SECTOR_SIZE != 512)
#error \
    "The current implementation of internal flash file system, only supports a media sector size of 512.  Please modify your selected value in the FSconfig.h file."
#endif

#if(FS_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT != 16) && (FS_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT != 32) && \
    (FS_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT != 48) && (FS_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT != 64)
