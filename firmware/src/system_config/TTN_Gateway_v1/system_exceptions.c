...
void _general_exception_handler ( void )
{
    ...
    ErrorMessageFatal_StoreGeneralException(_excep_code, _excep_addr);
    RebootWithMessage("General Exception %s (cause=%d, addr=%x)", 
                    _cause_str, _excep_code, _excep_addr);    
}

void _simple_tlb_refill_exception_handler(void)
{
    static unsigned int badEPC;
    static unsigned int badNestedEPC;

    badEPC = _CP0_GET_EPC();
    badNestedEPC = _CP0_GET_NESTEDEPC();

    ErrorMessageFatal_StoreInvalidPointer(badEPC, badNestedEPC);
    RebootWithMessage("Tbl refill exception (bad addr=%x, nested addr=%x)", 
                    badEPC, badNestedEPC);
}
