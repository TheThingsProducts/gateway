 /*******************************************************************************
  ...
  Summary:
    Clock system service implementation for PIC32 MK devices.

  Description:
   	This file Implements the functions specific to PIC32 MK devices.
*******************************************************************************/
...
#include "system/clk/src/sys_clk_PIC32MK.h"
                                          ...
                                          unsigned char *valueToPLLODIV )
{
    ...
    for ( multCount = 1; multCount <= 128; ++multCount )
    {
        ...
        {
            /* Implementation unique to MK chip family */
            if ( divCount == 0 )
            {
              divCount++;
            }
            ...
            uint32_t targetFreqHz, bool waitUntilComplete, uint32_t *clockClosest )
{
    ...
    switch ( referenceSource )
    {
        ...
        case CLK_SOURCE_REF_USBPLL_OUT:
            inputFreq = clkObject.usbClock[0];
            break;
