...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _DEVCON_CFG_LOCK_VREG(index)

  MASKs: 
    _DEVCON_CFG_LOCK_MASK(index)

  POSs: 
    _DEVCON_CFG_LOCK_POS(index)

  LENs: 
    _DEVCON_CFG_LOCK_LEN(index)
...
{
    _SFR_BIT_WRITE(_DEVCON_CFG_LOCK_VREG(index),_DEVCON_CFG_LOCK_POS(index),lockType);
}
...
{
    _SFR_BIT_WRITE(_DEVCON_CFG_LOCK_VREG(index),_DEVCON_CFG_LOCK_POS(index),0);
}
