...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DEVCON_IC_ALT_CLOCK_SELECT_VREG(index)
    _DEVCON_OC_ALT_CLOCK_SELECT_VREG(index)

  MASKs:
    _DEVCON_IC_ALT_CLOCK_SELECT_MASK(index)
    _DEVCON_OC_ALT_CLOCK_SELECT_MASK(index)

  POSs:
    _DEVCON_IC_ALT_CLOCK_SELECT_POS(index)
    _DEVCON_OC_ALT_CLOCK_SELECT_POS(index)

  LENs:
    _DEVCON_IC_ALT_CLOCK_SELECT_LEN(index)
    _DEVCON_OC_ALT_CLOCK_SELECT_LEN(index)
...
{
    switch ( targetAltClock )
    {
        case DEVCON_ALT_CLOCK_IC:
            _SFR_BIT_WRITE(_DEVCON_IC_ALT_CLOCK_SELECT_VREG(index),_DEVCON_IC_ALT_CLOCK_SELECT_POS(index),1);
            break;
        case DEVCON_ALT_CLOCK_OC:
            _SFR_BIT_WRITE(_DEVCON_OC_ALT_CLOCK_SELECT_VREG(index),_DEVCON_OC_ALT_CLOCK_SELECT_POS(index),1);
            break;
        default:
            PLIB_ASSERT(false,"Unknown targetAltClock!");
            break;
    }//end switch ( targetAltClock )
}
...
PLIB_TEMPLATE void DEVCON_AlternateClockDisable_Default( DEVCON_MODULE_ID index , DEVCON_ALT_CLOCK_TARGET targetAltClock )
{
    ...
    {
        case DEVCON_ALT_CLOCK_IC:
            _SFR_BIT_WRITE(_DEVCON_IC_ALT_CLOCK_SELECT_VREG(index),_DEVCON_IC_ALT_CLOCK_SELECT_POS(index),0);
            break;
        case DEVCON_ALT_CLOCK_OC:
            _SFR_BIT_WRITE(_DEVCON_OC_ALT_CLOCK_SELECT_VREG(index),_DEVCON_OC_ALT_CLOCK_SELECT_POS(index),0);
            break;
        default:
            PLIB_ASSERT(false,"Unknown targetAltClock!");
            break;
