...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _NVM_PROVIDE_QUAD_DATA0_VREG(index)

  MASKs: 
    _NVM_PROVIDE_QUAD_DATA0_MASK(index)

  POSs: 
    _NVM_PROVIDE_QUAD_DATA0_POS(index)

  LENs: 
    _NVM_PROVIDE_QUAD_DATA0_LEN(index)
...
PLIB_TEMPLATE void NVM_FlashProvideQuadData_Default( NVM_MODULE_ID index , uint32_t *data )
{
   ...
   for(flashData = 0; flashData < 4; flashData++)
   {
        _SFR_WRITE(_NVM_PROVIDE_QUAD_DATA0_VREG(index) + (flashData * 4) , *(data + flashData));
   }
