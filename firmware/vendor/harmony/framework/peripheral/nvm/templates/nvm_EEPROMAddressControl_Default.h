...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _NVM_EEPROM_ADDRESS_VREG(index)

  MASKs: 
    _NVM_EEPROM_ADDRESS_MASK(index)

  POSs: 
    _NVM_EEPROM_ADDRESS_POS(index)

  LENs: 
    _NVM_EEPROM_ADDRESS_LEN(index)
...
{
	uint32_t base_bits;
	base_bits = (address & 0x0003)
	
	if(base_bits || (address < 0x0000) || (address > 0x0FFC))
	{
		return false;
	}
	else
	{
		_SFR_WRITE( _NVM_EEPROM_ADDRESS_VREG( index ), address );
		return true;
	}
}
