...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _NVM_OPERATION_MODE_VREG(index)

  MASKs: 
    _NVM_OPERATION_MODE_MASK(index)

  POSs: 
    _NVM_OPERATION_MODE_POS(index)

  LENs: 
    _NVM_OPERATION_MODE_LEN(index)
...
PLIB_TEMPLATE void NVM_MemoryOperationSelect_Default( NVM_MODULE_ID index , NVM_OPERATION_MODE	operationmode )
{
    ...
    _SFR_FIELD_WRITE(_NVM_OPERATION_MODE_VREG(index),
					 _NVM_OPERATION_MODE_MASK(index),
					 _NVM_OPERATION_MODE_POS(index) ,
					 operationmode);
}
