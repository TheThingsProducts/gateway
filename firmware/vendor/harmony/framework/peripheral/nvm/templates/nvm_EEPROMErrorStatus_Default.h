...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _NVM_EEPROM_ERROR_STATUS_VREG(index)

  MASKs: 
    _NVM_EEPROM_ERROR_STATUS_MASK(index)

  POSs: 
    _NVM_EEPROM_ERROR_STATUS_POS(index)

  LENs: 
    _NVM_EEPROM_ERROR_STATUS_LEN(index)
...
{
    return (EEPROM_ERROR)(_SFR_FIELD_READ(_NVM_EEPROM_ERROR_STATUS_VREG(index),
					 _NVM_EEPROM_ERROR_STATUS_MASK(index),
					 _NVM_EEPROM_ERROR_STATUS_POS(index)));
}
...
{
    _SFR_FIELD_CLEAR(_NVM_EEPROM_ERROR_STATUS_VREG(index),
					 _NVM_EEPROM_ERROR_STATUS_MASK(index),
					 _NVM_EEPROM_ERROR_STATUS_POS(index));
}
