...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _NVM_LOCK_PFM_VREG(index)

  MASKs: 
    _NVM_LOCK_PFM_MASK(index)

  POSs: 
    _NVM_LOCK_PFM_POS(index)

  LENs: 
    _NVM_LOCK_PFM_LEN(index)
...
{
    _SFR_BIT_CLEAR(_NVM_LOCK_PFM_VREG(index),
                     _NVM_LOCK_PFM_POS(index) );
}
...
{
    return( !(_SFR_BIT_READ( _NVM_LOCK_PFM_VREG( index ),
                           _NVM_LOCK_PFM_POS( index ) ) ));
}
