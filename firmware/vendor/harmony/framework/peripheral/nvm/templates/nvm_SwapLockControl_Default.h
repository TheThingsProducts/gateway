...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _NVM_SWAP_LOCK_VREG(index)

  MASKs: 
    _NVM_SWAP_LOCK_MASK(index)

  POSs: 
    _NVM_SWAP_LOCK_POS(index)

  LENs: 
    _NVM_SWAP_LOCK_LEN(index)
...
{
     _SFR_FIELD_WRITE(_NVM_SWAP_LOCK_VREG(index),
					 _NVM_SWAP_LOCK_MASK(index),
					 _NVM_SWAP_LOCK_POS(index) ,
					 lockType);
}
...
{
    return (NVM_FLASH_SWAP_LOCK_TYPE)(_SFR_FIELD_READ(_NVM_SWAP_LOCK_VREG(index),
					 _NVM_SWAP_LOCK_MASK(index),
					 _NVM_SWAP_LOCK_POS(index) ));
}
