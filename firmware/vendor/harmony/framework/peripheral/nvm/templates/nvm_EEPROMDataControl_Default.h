...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _NVM_EEPROM_DATA_VREG(index)

  MASKs: 
    _NVM_EEPROM_DATA_MASK(index)

  POSs: 
    _NVM_EEPROM_DATA_POS(index)

  LENs: 
    _NVM_EEPROM_DATA_LEN(index)
...
{
    _SFR_WRITE( _NVM_EEPROM_DATA_VREG( index ), data );
}
...
{
    return (uint32_t)(_SFR_READ( _NVM_EEPROM_DATA_VREG( index )));
}
