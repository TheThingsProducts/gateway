...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    ...
    _PORTS_REMAP_FUNC_INT2_GROUPED_1_VREG(index)

  MASKs: 
    _PORTS_REMAP_FUNC_INT1_MASK(index)
    _PORTS_REMAP_FUNC_INT2_GROUPED_1_MASK(index)

  POSs: 
    _PORTS_REMAP_FUNC_INT1_POS(index)
    _PORTS_REMAP_FUNC_INT2_GROUPED_1_POS(index)

  LENs: 
    _PORTS_REMAP_FUNC_INT1_LEN(index)
    _PORTS_REMAP_FUNC_INT2_GROUPED_1_LEN(index)
...
PLIB_TEMPLATE void PORTS_RemapInput_grouped1( PORTS_MODULE_ID      index , PORTS_REMAP_INPUT_FUNCTION inputFunction , PORTS_REMAP_INPUT_PIN      remapInputPin )
{
    ...
    _SFR_FIELD_WRITE( _PORTS_REMAP_FUNC_INT1_VREG(index) + inputFunction,
                          _PORTS_REMAP_FUNC_INT1_MASK(index),
                          _PORTS_REMAP_FUNC_INT1_POS(index),
                            remapInputPin );
}
