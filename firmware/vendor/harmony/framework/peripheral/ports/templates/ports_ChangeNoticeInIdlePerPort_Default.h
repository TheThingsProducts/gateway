...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_VREG(index)

  MASKs: 
    _PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_MASK(index)

  POSs: 
    _PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_POS(index)

  LENs: 
    _PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_LEN(index)
...
{
    _SFR_BIT_CLEAR(_PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_VREG(index) + ((channel-1) * 0x40),
		                           _PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_POS(index) );
}
...
{
    _SFR_BIT_SET(_PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_VREG(index) + ((channel-1) * 0x40),
						   _PORTS_PIN_CHANGE_NOTICE_IN_IDLE_B_POS(index) );
}
