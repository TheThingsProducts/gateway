...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _PORTS_REMAP_PIN_RPA0_VREG(index)

  MASKs: 
    _PORTS_REMAP_PIN_RPA0_MASK(index)

  POSs: 
    _PORTS_REMAP_PIN_RPA0_POS(index)

  LENs: 
    _PORTS_REMAP_PIN_RPA0_LEN(index)
...
{
    _SFR_FIELD_WRITE( _PORTS_REMAP_PIN_RPA0_VREG(index) + remapOutputPin,
					  _PORTS_REMAP_PIN_RPA0_MASK(index),
					  _PORTS_REMAP_PIN_RPA0_POS(index),
					  outputFunction );
}
