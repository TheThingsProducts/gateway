...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _INT_PRIORITY_SHADOW_REGISTER_SELECT_VREG(index)

  MASKs:
    _INT_PRIORITY_SHADOW_REGISTER_SELECT_MASK(index)

  POSs:
    _INT_PRIORITY_SHADOW_REGISTER_SELECT_POS(index)

  LENs:
    _INT_PRIORITY_SHADOW_REGISTER_SELECT_LEN(index)
...
{
    _SFR_SET(_INT_PRIORITY_SHADOW_REGISTER_SELECT_VREG(index), (shadowRegister << (priority * 4)));
}
...
{
    return (INT_SHADOW_REGISTER)(_SFR_READ(_INT_PRIORITY_SHADOW_REGISTER_SELECT_VREG(index)) >> ( priority * 4 ));
}
