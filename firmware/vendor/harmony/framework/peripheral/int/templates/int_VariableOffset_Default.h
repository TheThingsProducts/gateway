...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _INT_VECTOR_ADDRESS_OFFSET_SET_VREG(index)

  MASKs:
    _INT_VECTOR_ADDRESS_OFFSET_SET_MASK(index)

  POSs:
    _INT_VECTOR_ADDRESS_OFFSET_SET_POS(index)

  LENs:
    _INT_VECTOR_ADDRESS_OFFSET_SET_LEN(index)
...
{
    _SFR_WRITE((_INT_VECTOR_ADDRESS_OFFSET_SET_VREG(index) + (vector)), offset );
}
...
{
    return _SFR_READ(_INT_VECTOR_ADDRESS_OFFSET_SET_VREG(index)+ ( vector));
}
