...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _INT_SOFTWARE_NMI_VREG(index)

  MASKs: 
    _INT_SOFTWARE_NMI_MASK(index)

  POSs: 
    _INT_SOFTWARE_NMI_POS(index)

  LENs: 
    _INT_SOFTWARE_NMI_LEN(index)
...
{
    _SFR_FIELD_WRITE( _INT_SOFTWARE_NMI_VREG(index), _INT_SOFTWARE_NMI_MASK(index), _INT_SOFTWARE_NMI_POS(index), 0x4E );
}
