...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _ETH_FRAMES_RCVD_OK_COUNT_VREG(index)

  MASKs:
    _ETH_FRAMES_RCVD_OK_COUNT_MASK(index)

  POSs:
    _ETH_FRAMES_RCVD_OK_COUNT_POS(index)

  LENs:
    _ETH_FRAMES_RCVD_OK_COUNT_LEN(index)
...
{
    _SFR_WRITE(_ETH_FRAMES_RCVD_OK_COUNT_VREG(index),0x0000);
}
...
{
    return (uint16_t)_SFR_READ(_ETH_FRAMES_RCVD_OK_COUNT_VREG(index));
}
