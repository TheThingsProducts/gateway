...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _ETH_RX_START_ADDRESS_VREG(index)

  MASKs:
    _ETH_RX_START_ADDRESS_MASK(index)

  POSs:
    _ETH_RX_START_ADDRESS_POS(index)

  LENs:
    _ETH_RX_START_ADDRESS_LEN(index)
...
{
    _SFR_WRITE(_ETH_RX_START_ADDRESS_VREG(index),(uintptr_t)rxPacketDescStartAddr);
}
...
{
    return (uint8_t *)_SFR_READ(_ETH_RX_START_ADDRESS_VREG(index));
}
