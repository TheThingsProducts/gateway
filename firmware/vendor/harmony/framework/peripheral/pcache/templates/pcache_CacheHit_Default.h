...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _PCACHE_CACHE_HIT_VREG(index)

  MASKs: 
    _PCACHE_CACHE_HIT_MASK(index)

  POSs: 
    _PCACHE_CACHE_HIT_POS(index)

  LENs: 
    _PCACHE_CACHE_HIT_LEN(index)
...
{
    return _SFR_READ(_PCACHE_CACHE_HIT_VREG(index));
}
...
{
    _SFR_WRITE(_PCACHE_CACHE_HIT_VREG(index), data);
}
