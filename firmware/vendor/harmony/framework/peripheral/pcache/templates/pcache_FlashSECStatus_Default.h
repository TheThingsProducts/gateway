...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _PCACHE_FLASH_SEC_STATUS_VREG(index)

  MASKs: 
    _PCACHE_FLASH_SEC_STATUS_MASK(index)

  POSs: 
    _PCACHE_FLASH_SEC_STATUS_POS(index)

  LENs: 
    _PCACHE_FLASH_SEC_STATUS_LEN(index)
...
{
    return _SFR_BIT_READ(_PCACHE_FLASH_SEC_STATUS_VREG(index),
                         _PCACHE_FLASH_SEC_STATUS_POS(index));
}
...
{
    _SFR_BIT_SET(_PCACHE_FLASH_SEC_STATUS_VREG(index), _PCACHE_FLASH_SEC_STATUS_POS(index));
}
...
{
    _SFR_BIT_CLEAR(_PCACHE_FLASH_SEC_STATUS_VREG(index), _PCACHE_FLASH_SEC_STATUS_POS(index));
}
