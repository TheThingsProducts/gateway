...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _TMR_PERIOD_MATCH_MODE_VREG(index)

  MASKs: 
    _TMR_PERIOD_MATCH_MODE_MASK(index)

  POSs: 
    _TMR_PERIOD_MATCH_MODE_POS(index)

  LENs: 
    _TMR_PERIOD_MATCH_MODE_LEN(index)
...
PLIB_TEMPLATE bool TMR_IsPeriodMatchBased_Default( TMR_MODULE_ID index )
{
    ...
    period = (uint16_t)_SFR_READ( _TMR_PERIOD_MATCH_MODE_VREG( index ) );
