...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_VREG(index)
    _OSC_OSC_REGISTER_LOCK_VREG(index)

  MASKs: 
    _OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_MASK(index)
    _OSC_OSC_REGISTER_LOCK_MASK(index)

  POSs: 
    _OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_POS(index)
    _OSC_OSC_REGISTER_LOCK_POS(index)

  LENs: 
    _OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_LEN(index)
...
{
    _SFR_FIELD_WRITE(_OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_VREG(index),
                     _OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_MASK(index),
                     _OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_POS(index) ,
                     PLLInClockSource);
...
{
     return (OSC_SYSPLL_IN_CLK_SOURCE)_SFR_FIELD_READ(_OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_VREG(index),
     						_OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_MASK(index),
                            _OSC_OSC_BTPLL_INPUT_SOURCE_CLOCK_POS(index) );
}
