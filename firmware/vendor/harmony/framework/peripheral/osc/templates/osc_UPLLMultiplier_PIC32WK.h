...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OSC_OSC_UPLL_MULTIPLIER_VREG(index)
    _OSC_OSC_REGISTER_LOCK_VREG(index)

  MASKs: 
    _OSC_OSC_UPLL_MULTIPLIER_MASK(index)
    _OSC_OSC_REGISTER_LOCK_MASK(index)

  POSs: 
    _OSC_OSC_UPLL_MULTIPLIER_POS(index)
    _OSC_OSC_REGISTER_LOCK_POS(index)

  LENs: 
    _OSC_OSC_UPLL_MULTIPLIER_LEN(index)
...
PLIB_TEMPLATE void OSC_UPLLMultiplierSelect_PIC32WK( OSC_MODULE_ID index , OSC_SYSPLL_MULTIPLIER_TYPE pll_multiplier )
{
	...
	{
		_SFR_FIELD_WRITE(_OSC_OSC_UPLL_MULTIPLIER_VREG(index),
						 _OSC_OSC_UPLL_MULTIPLIER_MASK(index),
						 _OSC_OSC_UPLL_MULTIPLIER_POS(index) ,
						 (pll_multiplier - 1) );
	}
...
{
    return ( _SFR_FIELD_READ(_OSC_OSC_UPLL_MULTIPLIER_VREG(index),
                           _OSC_OSC_UPLL_MULTIPLIER_MASK(index),
                           _OSC_OSC_UPLL_MULTIPLIER_POS(index) ) + 1);
}
