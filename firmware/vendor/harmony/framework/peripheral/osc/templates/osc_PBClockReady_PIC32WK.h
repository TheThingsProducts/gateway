...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OSC_OSC_PB_CLOCK_READY_WK_1_VREG(index)
    _OSC_OSC_PB_CLOCK_READY_WK_2_VREG(index)
    _OSC_OSC_PB_CLOCK_READY_WK_3_VREG(index)
    _OSC_OSC_PB_CLOCK_READY_WK_4_VREG(index)

  MASKs: 
    _OSC_OSC_PB_CLOCK_READY_WK_1_MASK(index)
    _OSC_OSC_PB_CLOCK_READY_WK_2_MASK(index)
    _OSC_OSC_PB_CLOCK_READY_WK_3_MASK(index)
    _OSC_OSC_PB_CLOCK_READY_WK_4_MASK(index)

  POSs: 
    _OSC_OSC_PB_CLOCK_READY_WK_1_POS(index)
    _OSC_OSC_PB_CLOCK_READY_WK_2_POS(index)
    _OSC_OSC_PB_CLOCK_READY_WK_3_POS(index)
    _OSC_OSC_PB_CLOCK_READY_WK_4_POS(index)

  LENs: 
    _OSC_OSC_PB_CLOCK_READY_WK_1_LEN(index)
    _OSC_OSC_PB_CLOCK_READY_WK_2_LEN(index)
    _OSC_OSC_PB_CLOCK_READY_WK_3_LEN(index)
    _OSC_OSC_PB_CLOCK_READY_WK_4_LEN(index)
...
{
   return _SFR_BIT_READ(_OSC_OSC_PB_CLOCK_READY_WK_1_VREG(index) + ((peripheralBusNumber-1) * 0x04),
                             _OSC_OSC_PB_CLOCK_READY_WK_1_POS(index) );
}
