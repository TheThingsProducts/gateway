...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _USART_BAUD_CLOCK_SELECT_VREG(index)

  MASKs: 
    _USART_BAUD_CLOCK_SELECT_MASK(index)

  POSs: 
    _USART_BAUD_CLOCK_SELECT_POS(index)

  LENs: 
    _USART_BAUD_CLOCK_SELECT_LEN(index)
...
{
   _SFR_FIELD_WRITE(_USART_BAUD_CLOCK_SELECT_VREG(index),
                    _USART_BAUD_CLOCK_SELECT_MASK(index),
                    _USART_BAUD_CLOCK_SELECT_POS(index),
                    brgClockSource);
}
...
{
    return (USART_BRG_CLOCK_SOURCE)_SFR_FIELD_READ(_USART_BAUD_CLOCK_SELECT_VREG(index),
                           _USART_BAUD_CLOCK_SELECT_MASK(index),
                           _USART_BAUD_CLOCK_SELECT_POS(index));
}
