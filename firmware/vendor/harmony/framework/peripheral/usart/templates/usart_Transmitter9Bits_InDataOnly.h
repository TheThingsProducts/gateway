...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _USART_TRANSMITTER_DATA_ONLY_VREG(index)

  MASKs:
    _USART_TRANSMITTER_DATA_ONLY_MASK(index)

  POSs:
    _USART_TRANSMITTER_DATA_ONLY_POS(index)

  LENs:
    _USART_TRANSMITTER_DATA_ONLY_LEN(index)
...
PLIB_TEMPLATE void USART_Transmitter9BitsSend_InDataOnly( USART_MODULE_ID index , int8_t data , bool Bit9th )
{
	...
	if (Bit9th == 0)
	{
		_SFR_WRITE(_USART_TRANSMITTER_DATA_ONLY_VREG(index),
    			data);
	}
	else
	{
