...
typedef enum
{
    ...
    /* Note: that this is a private TCPIP heap */
    ...
    /* Notes: 
        - that this is a private TCPIP heap
          and multi-threaded protection is provided internally.
        - This heap is not supported yet */
    TCPIP_STACK_HEAP_TYPE_INTERNAL_POOL, 
...
typedef enum
{
    ...
    // TCPIP_STACK_HEAP_TYPE_INTERNAL_POOL type specific flag:
...
{
    // the TCPIP_STACK_HEAP_CONFIG members
    TCPIP_STACK_HEAP_TYPE   heapType;       // type of this heap: TCPIP_STACK_HEAP_TYPE_INTERNAL_POOL
    TCPIP_STACK_HEAP_FLAGS  heapFlags;      // heap creation flags
                                            // TCPIP_STACK_HEAP_FLAG_ALLOC_UNCACHED will be always internally set 
                                            //
    ...
    size_t                  heapSize;       // total size of the pool heap to be created and maintained

    int                     nPoolEntries;   // number of pool entries that the pool will contain
...
/*
  ...
  Parameters:
    heapType    - type of heap to request a handle for
                    Currently only TCPIP_STACK_HEAP_TYPE_INTERNAL_HEAP is supported
