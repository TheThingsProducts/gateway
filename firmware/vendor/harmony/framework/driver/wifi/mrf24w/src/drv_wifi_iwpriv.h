...
#ifndef _DRV_WIFI_IWPRIV_H
#define _DRV_WIFI_IWPRIV_H
...
#if defined(DRV_WIFI_IWPRIV_TASK)
...
/*
  ...
  Remarks:
    ...
    After the the module is connected, MRF24WG can also do regular scans in
    Infrastructure mode or Ad-Hoc mode, but it cannot perform a scan in Soft AP
    mode. It also cannot perform a scan when the connection is in progress.
 */
void iwpriv_prescan_option_set(bool scan);
...
/*
  ...
  Remarks:
    IWPRIV_CONNECTION_FAILED does not necessarily mean
    DRV_WIFI_EVENT_CONNECTION_FAILED. It stands on the application's
    perspective, and actually can be customized. For example, in the Web Server
    demonstration's use case, DRV_WIFI_EVENT_CONNECTION_PERMANENTLY_LOST is treated as a
    fail case and will trigger the application to restart.
 */
IWPRIV_CONN_STATUS iwpriv_connstatus_get(void);
...
/*
  ...
  Description:
    This function returns the device information, which is either MRF24WG or
    MRF24WN.
  ...
  Remarks:
    This function is different from function DRV_WIFI_DeviceInfoGet().
 */
void iwpriv_devinfo_get(void *info);
...
/*
  ...
  Description:
    This function sets the current Ad-Hoc network context information by reading
    from a passed pointer.
