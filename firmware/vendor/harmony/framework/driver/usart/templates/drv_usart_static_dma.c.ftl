...
<#if CONFIG_DRV_USART_SUPPORT_TRANSMIT_DMA == true>
    ...
    if( DMA_CHANNEL_NONE != ${XMIT_DMA_CHANNEL} &&
        DMA_NUMBER_OF_CHANNELS != ${XMIT_DMA_CHANNEL})
    {
        ...
        if(SYS_DMA_CHANNEL_HANDLE_INVALID != dObj->dmaChannelHandleWrite)
        {
            ...
            SYS_DMA_ChannelSetup(dObj->dmaChannelHandleWrite,
                SYS_DMA_CHANNEL_OP_MODE_BASIC, ${XMIT_INT});
        }
    }
</#if>
<#if CONFIG_DRV_USART_SUPPORT_RECEIVE_DMA == true>
    ...
    if(DMA_CHANNEL_NONE != ${RCV_DMA_CHANNEL} &&
        DMA_NUMBER_OF_CHANNELS != ${RCV_DMA_CHANNEL})
    {
...
<#else>
...
</#if>
    {
        ...
        if(SYS_DMA_CHANNEL_HANDLE_INVALID == gDrvUSART${DRV_INSTANCE}Obj.dmaChannelHandleWrite)
        {
...
<#else>
...
</#if>
    {
        ...
        /* Checking whether the transfer uses DMA channels */
        if(SYS_DMA_CHANNEL_HANDLE_INVALID == gDrvUSART${DRV_INSTANCE}Obj.dmaChannelHandleRead)
        {
            /* Clear up the interrupt flag */
            SYS_INT_SourceStatusClear(${RCV_INT});
        }
