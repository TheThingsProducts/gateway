...
bool DRV_USART_ReceiverBufferIsEmpty( const DRV_HANDLE handle )
{
    ...
    /* If there is data available in the receiver, then this function
       buffer is not empty and this function returns false. */

    return(PLIB_USART_ReceiverDataIsAvailable(hDriver->moduleId) ? false : true);
}
...
bool DRV_USART_TransmitBufferIsFull( const DRV_HANDLE handle )
{
    ...
    return(PLIB_USART_TransmitterBufferIsFull(hDriver->moduleId));
}
