...
)
{
    ...
    if( DMA_CHANNEL_NONE != usartInit->dmaChannelTransmit &&
        DMA_NUMBER_OF_CHANNELS != usartInit->dmaChannelTransmit)
    {
        ...
        if(SYS_DMA_CHANNEL_HANDLE_INVALID != dObj->dmaChannelHandleWrite)
        {
            ...
            SYS_DMA_ChannelSetup(dObj->dmaChannelHandleWrite,
                SYS_DMA_CHANNEL_OP_MODE_BASIC, usartInit->interruptTransmit);
            dObj->dmaInterruptTransmit = usartInit->dmaInterruptTransmit;
        }
    }
    ...
    if(DMA_CHANNEL_NONE != usartInit->dmaChannelReceive &&
        DMA_NUMBER_OF_CHANNELS != usartInit->dmaChannelReceive)
    {
        ...
        if(SYS_DMA_CHANNEL_HANDLE_INVALID != dObj->dmaChannelHandleRead)
        {
            ...
            SYS_DMA_ChannelSetup(dObj->dmaChannelHandleRead,
                SYS_DMA_CHANNEL_OP_MODE_BASIC, usartInit->interruptReceive);
            dObj->dmaInterruptReceive = usartInit->dmaInterruptReceive;
        }
    }
    ...
    SYS_INT_SourceStatusClear(dObj->dmaInterruptTransmit);
    SYS_INT_SourceStatusClear(dObj->dmaInterruptReceive);
    ...
    /* Create the hardware instance mutex. */
     ...
     {
        SYS_DEBUG(0, "Unable to create hardware instance mutex");
        return SYS_MODULE_OBJ_INVALID;
    }
    /* Check if the global mutexes have been created. If not
     ...
     if(!gDrvUSARTCommonDataObj.membersAreInitialized)
     {
         ...
         {
               SYS_DEBUG(0, "Unable to create client instance mutex");
        	   return SYS_MODULE_OBJ_INVALID;
		 }
         if(OSAL_MUTEX_Create(&(gDrvUSARTCommonDataObj.mutexBufferQueueObjects)) != OSAL_RESULT_TRUE)
         {
               SYS_DEBUG(0, "Unable to create buffer queue objects mutex");
         	    return SYS_MODULE_OBJ_INVALID;
		}
...
void  DRV_USART_Deinitialize(SYS_MODULE_OBJ object)
{
    ...
    status = _DRV_USART_InterruptSourceDisable(dObj->dmaInterruptTransmit);
    status = _DRV_USART_InterruptSourceDisable(dObj->dmaInterruptReceive);
    ...
    {
        SYS_DEBUG(0, "Failed to delete Mutex");
        return;
    }
    ...
    if(OSAL_MUTEX_Lock(&(gDrvUSARTCommonDataObj.mutexClientObjects), OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
    {
        ...
        for(iClient = 0; iClient != DRV_USART_CLIENTS_NUMBER; iClient ++)
        {
            if(!gDrvUSARTClientObj[iClient].inUse)
            {
                ...
                {
					    SYS_DEBUG(0, "Unable to create client read done semaphore");
        				return(DRV_HANDLE_INVALID);
				}
                if(OSAL_SEM_Create(&(clientObj->semWriteDone), OSAL_SEM_TYPE_COUNTING, 1, 0) != OSAL_RESULT_TRUE)
                {
					    SYS_DEBUG(0, "Unable to create client write done semaphore");
        				return(DRV_HANDLE_INVALID);
				}
...
void DRV_USART_Close ( DRV_HANDLE handle)
{
    ...
    {
        SYS_DEBUG(0, "Failed to delete Semaphore");
        return;
    }
...
void DRV_USART_TasksTransmit(SYS_MODULE_OBJ object)
{
    ...
    if(SYS_INT_SourceStatusGet(hDriver->txInterruptSource) ||
       SYS_INT_SourceStatusGet(hDriver->dmaInterruptTransmit))
    {
...
void DRV_USART_TasksReceive(SYS_MODULE_OBJ object)
{
    ...
    if(SYS_INT_SourceStatusGet(hDriver->rxInterruptSource) ||
       SYS_INT_SourceStatusGet(hDriver->dmaInterruptReceive))
    {
...
// *****************************************************************************
/* void _DRV_USART_HardwareSetup(USART_MODULE_ID  plibID,
                                        DRV_USART_INIT * init)
...
void _DRV_USART_HardwareSetup(USART_MODULE_ID plibID, DRV_USART_INIT * init)
{
    ...
    PLIB_USART_InitializeOperation(plibID, USART_RECEIVE_FIFO_ONE_CHAR,
            USART_TRANSMIT_FIFO_NOT_FULL, handshakeLines);
...
bool _DRV_USART_ClientBufferQueueObjectsRemove(DRV_USART_CLIENT_OBJ * clientObj)
{
    ...
    bool dmaInterruptWasEnabled = false;
    ...
    if(OSAL_MUTEX_Lock(&(dObj->mutexDriverInstance), OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
    {
        ...
        dmaInterruptWasEnabled = _DRV_USART_InterruptSourceDisable(dObj->dmaInterruptTransmit);
        iterator = dObj->queueWrite;
        while(iterator != NULL)
        {
        ...
        if(dmaInterruptWasEnabled)
        {
            _DRV_USART_InterruptSourceEnable(dObj->dmaInterruptTransmit);
        }
        ...
        dmaInterruptWasEnabled = _DRV_USART_InterruptSourceDisable(dObj->dmaInterruptReceive);
        iterator = dObj->queueRead;
        while(iterator != NULL)
        {
            if(clientObj == (DRV_USART_CLIENT_OBJ *)iterator->hClient)
        ...
        if(dmaInterruptWasEnabled)
        {
            _DRV_USART_InterruptSourceEnable(dObj->dmaInterruptReceive);
        }
        /* Unlock the mutex */
...
void _DRV_USART_BufferQueueRxTasks(SYS_MODULE_OBJ object)
{
    ...
    if(SYS_DMA_CHANNEL_HANDLE_INVALID != hDriver->dmaChannelHandleRead)
    {
        ...
        //    status = _DRV_USART_InterruptSourceDisable(hDriver->dmaInterruptReceive);
            /* Ignore the warning */
            //(void)status;
        }
        /* Release the mutex */
...
void _DRV_USART_BufferQueueTxTasks(SYS_MODULE_OBJ object)
{
    ...
    if(SYS_DMA_CHANNEL_HANDLE_INVALID != hDriver->dmaChannelHandleWrite)
    {
        ...
        if(hDriver->queueWrite == NULL)
        {
           ...
           // status = _DRV_USART_InterruptSourceDisable(hDriver->dmaInterruptTransmit);
           /* Ignore the warning */
           // (void)status;
        }
        /* Release the mutex */
